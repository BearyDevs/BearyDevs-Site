  ...
  return (
    <ScrollShadow
      ...
      className={classNames({
        "sticky top-0 z-50 mb-3 flex select-none items-center rounded-md mobilexll:pb-1 py-1":
          true,
      ...
      {menu.map((item) => {
        ...
        return (
          <Link
            key={item.title}
            href={item.href}
            className={`flex shrink-0 text-gray-300 items-center mt-2 lg:py-1 py-2 gap-1.5 px-3 leading-none transition-all lg:hover:bg-[#103f3c4f] ${isActive ? "bg-[#103f3cb5] text-white" : "bg-transparent"}`}
          >
